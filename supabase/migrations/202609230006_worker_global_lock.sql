...
grant execute on function public.liberar_lock_worker(bigint, text) to service_role;-- Fase 4: impede execuções sobrepostas do worker em processos diferentes.
-- Aplicar manualmente no Supabase SQL Editor.
--
-- A concessão persistida é o mecanismo de coordenação. O token impede que
-- uma execução antiga libere a concessão de uma execução nova depois de
